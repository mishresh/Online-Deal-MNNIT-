...
where book.book_id = notification.book_id and notification.to_reg_no=register.reg_no and from_reg_no = '$reg_no'" ;   
...
<div style="padding-center:50px; padding-left:50px; color: MidnightBlue; font-size: 20px; text-shadow: -1px 0 black, 0 1px OrangeRed, 1px 0 gold, 0 -1px red;">
  <h2>Total Requested Books !!!</h2>
  </div></div>
  <!-- new edition
   -->
   <div style="color:black ;border-radius: 20px; padding: 50px 100px 50px 15px;background-color:white;opacity: 0.8; ">
    ...
    while ($row = mysqli_fetch_array($query))
    {
      ...
      echo '<tr>
         ...
         <td>'.$row['request_status'].'</td>
          <td><form action="deleterequest.php" method="POST"><input style="display:none" type="text" name="passval" value="'.$row['book_book_id'].'"><center><input button class="button button2" type="submit" value="DELETE" name="'.$a.'"></center></form></td>
        </tr>';
